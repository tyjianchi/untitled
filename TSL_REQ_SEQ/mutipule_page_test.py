...
class Application(tk.Tk):
    ...
    def __init__(self):
        ...
        self.frames = {}
        for F in (IndexPage,StartPage, PageOne):
...
class IndexPage(tk.Frame):
    def __init__(self,parent,root):
        ...
        label=tk.Label(self,text="老板，欢迎光临！")
        label.pack()
        button1 = ttk.Button(self, text="可供记账模板", command=lambda: root.show_frame(StartPage)).pack()
        self.L8 = tk.Label(self)
        self.L8["text"] = "---------------------------------------"
        self.L8.pack()
        button2 = ttk.Button(self, text="可供出售金融资产", command=lambda: root.show_frame(PageOne)).pack()
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, root):
        super().__init__(parent)
        label = tk.Label(self, text="可供记账模板", font=LARGE_FONT)
        label.pack(pady=10, padx=10)
        button1 = ttk.Button(self, text="回到主页", command=lambda: root.show_frame(IndexPage)).pack()
    ...
    def openfiles2(self):
        gl=gl_model()
        tk.messagebox.showinfo('提示','请确保文件内容<可供出售账户每月估值表>')
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, root):
        super().__init__(parent)
        label = tk.Label(self, text="可供出售金融资产", font=LARGE_FONT)
        label.pack(pady=10, padx=10)

        self.E7 = tk.Button(self)
        self.E7["text"] = "选择文件"
        self.E7["command"] = self.openfiles2
        self.E7.pack()
    ...
    def openfiles2(self):
        ...
        tk.messagebox.showinfo('提示','请确保文件内容<可供出售序时表>')

        s2fname = filedialog.askopenfilename(title='打开Excel文件(xls)',
                                             filetypes=[('xls', '*.xls')])
        filename=os.path.split(s2fname)[1]
        if filename:
           gl.pd_read(s2fname)

        else:
           tk.messagebox.showerror('错误','未选中文件!')
...
class gl_model():
    ...
    def get_code(self,item_name):
          reg_list=  {"铁道债": "铁道",
                      ...
                      "银行CD": "银行CD",
                      "政策债": ("国开", "进出","农发"),
                      "地方债": ("北京","天津","上海","重庆","河北","山西","辽宁","吉林","黑龙江","江苏","浙江","安徽","福建","江西","山东","河南","湖北","湖南","广东","海南","四川","贵州","云南","陕西","甘肃","青海","台湾","内蒙古自治区","广西壮族自治区","西藏自治区","宁夏回族自治区","新疆维吾尔自治区","香港特别行政区","澳门特别行政区","大连"),
                      "企业债": ("嘉禾铸都","韶山高新")}
...
class gl_model2():
    ...
    def pd_read(self,filepath):
        try:
            ...
            #写入文件
            sq_file=os.path.split(filepath)[0]+'/SQ_ALL.xlsx'
            df_pivoted.to_excel(sq_file, sheet_name='Data1')
            tk.messagebox.showinfo('提示',sq_file+'：文件已生成！')

        except Exception:
            tk.messagebox.showinfo('提示','错误：'+traceback.format_exc())
